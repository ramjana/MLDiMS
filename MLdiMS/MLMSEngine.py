...
class MLMSEngine():
    ...
    def __init__(self,config: llmConfig):
        ...
        #model definition
        self.model = models.Transformer(config=self.config,mesh=self.mesh)
    ...
    def prefill(self,
        ...
        with self.mesh, nn_partitioning.axis_rules(self.config_logical_axis_rules):
            _logits, new_kvstates = self.model.apply(
                    params,
                    input_tokens,
                    positions,
                    decoder_segments_ids = sequence_indicator,
                    enable_dropout=True,
                    mode="prefill",
                    rngs={"params": new_rng},
                    mutable=["kvcache"],
            )
        first_token = return jnp.argmax(_logits, axis=-1)
    ...
    def append(
        ...
        new_token = return jnp.argmax(_logits, axis=-1)
    ...
    def insert(
        ...
        unboxed_prefix = engine_utils.unbox_logicallypartioned(prefix)

        def copy(path, partial_cache, full_cache, annotations):
            path_key = path[-1].key
            if path_key in ["kvcached_append_index", "cached_append_key", "kvcached_append_value"]:
        ...
        inserted_cache = jax.tree_util.tree_map_with_path(
            copy, unboxed_prefix["kvcache"], decode_state["kvcache"], self.kv_cache_annotations_named
        )
        ...
        def get_tokenizer(self):
            ...
            file_path = os.path.dirname(dir_path,"configs") 
        ...
        def init_decode_state(
            ...
            del cache
            zeroed = engine_utils.unbox_logicallypartioned(initialize())
            return zeroed
