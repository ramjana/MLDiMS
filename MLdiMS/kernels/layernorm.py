...
def layernorm(M: int, K: int,
    ...
    #      wait_for_data
    #      welford_op(payload)  & prefetch payload[packIdx++]
    ...
    def  welford_op(num_elements:int, bpe:int=2):
        #0  f16->f32 if f16
        #1. delta = elelment - mean     fsub   
        #2. count = increment mean_count add
        #3. scale = 1/count              frcp   (transcedental)
        #4. new mean += delta/count      fma
        #5. delta1 = element - new_mean  fsub
        #6  m2 += delta*delta1           fma
        
        #5 ops using mul/fma rate and f_rcp transcedental ops
        num_ops = 5
        if bpe == 2:
            num_ops += 1
        cycles = num_ops * math.ceil(num_elements/hwCfg.mul32_rate) + 1 * math.ceil(num_elements/hwCfg.transcendental_rate)
        #print(f"welford_step {payload} {cycles}")
        return cycles
    ...
    def shuffle_wave_reduction(lanes_per_row:int, blocksize:int, bpe):
        ...
        for iter in range(int(num_steps + sdwa_step)):
           ...
           latency += welford_op(payload,bpe)
        ##broadcast
    
        latency += (blocksize//256)*4*3
        return latency
    ...
    def welford_wg_reduction(num_wave32_per_row:int,bpe:int):
        writepayload_lat = ((BlockSize//64) * 3)*12    #mean, m2, count
    ...
    vec_alu_cycles = welford_op(vec_payload,bpe)
    ln_alu_cycles = welford_op(payload,bpe)
    ...
    if (num_threads_per_row > 32):
        wg_reduction_cycles = welford_wg_reduction(num_threads_per_row//32, bpe)
        
    print(f"ln wave_reduction {wave_reduction_cycles} wg reduction = {wg_reduction_cycles}")
        
    if (ln_alu_cycles > memrd_cycles): 
        #not optimized version
        ln_cycles = ((memrd_cycles + welford_op(vec_payload,bpe)) *col_loop_iter * row_loop_iter) 
        ln_cycles += (wave_reduction_cycles + wg_reduction_cycles)* row_loop_iter * num_row_packs
        sw_alu_cycles = 0 if countin_uint == False else countin_int(BlockSize*num_row_packs)
        ...
        ln_cycles += 0 if countin_uint == False else countin_int(BlockSize*num_row_packs)
    ...
    if (apply_GSU_reduction):
        ...
        mall_write_cycles= hwCfg.l2_miss_latency + hwCfg.mall_hit_latency + math.ceil((num_rows * 3 * 4 * (M)* GSU)/(hwCfg.mallwr_bw_cu * hwCfg.mall_efficiency))
        ...
        mall_read_cycles =  hwCfg.l2_miss_latency + hwCfg.mall_hit_latency + math.ceil((num_rows * 3 * 4 * (M)* GSU)/(hwCfg.mallrd_bw_cu * hwCfg.mall_efficiency))
        GSU_cycles = mall_read_cycles + mall_write_cycles + atomic_latency + welford_op(GSU*M,bpe)
    ...
    if (fusedOps != None):
        ...
        # v_rsq_f32   take inverse of variance 
        # f_mul variance * mean
        # fma  element, sqrt(variance), -variance*mean  
        count_inv  = (BlockSize // 64) * 4 * (num_rows//num_rows_per_access)
        std_cycles = (BlockSize // 64 ) * 4  * (num_rows//num_rows_per_access)  + count_inv
        # fsub + fmul
        ln_math_cycles = ((payload * col_loop_iter * row_loop_iter)//64) * 4  + ((payload * col_loop_iter * row_loop_iter)//64)  * 4
        
        ln_cycles += ln_math_cycles + std_cycles
...
if __name__ == "__main__":
    ...
    """
    cycles = layernorm(4096*200*64,64,jnp.float16,hwCfg=hwConfig,algoCfg=lnConfig,
